 ...
 * `Correction` is the difference of actual frequency and desired frequency @
 * 100 MHz. It can be measured at lower frequencies and scaled linearly. E.g. if
 * you get 10_000_097 Hz instead of 10_000_000 Hz, `correction` is 97*10 = 970
 */
...
void si5351_Calc(int32_t Fclk, si5351PLLConfig_t *pll_conf,
  ...
  Fclk = Fclk -
         (int32_t)((((double)Fclk) / 100000000.0) * ((double)si5351Correction));
...
void si5351_write(uint8_t reg, uint8_t value) {
  while (HAL_I2C_IsDeviceReady(&I2C_HANDLE, (uint16_t)(SI5351_ADDRESS << 1), 3,
