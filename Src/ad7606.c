...
// SPI 16-bit mode, single data lane, hw nSS, nSSP Enabled, CPOL high, CPHA 2
// edge
...
void AD7606_Init(AD7606_Handler *device, SPI_HandleTypeDef *spi_handler,
                 TIM_HandleTypeDef *conva_tim_handler, uint32_t conva_channel,
                 TIM_HandleTypeDef *convb_tim_handler, uint32_t convb_channel,
                 GPIO_TypeDef *busy_gpio_port, uint16_t busy_gpio_pin,
                 GPIO_TypeDef *reset_gpio_port, uint16_t reset_gpio_pin,
...
#else
void AD7606_Init(AD7606_Handler *device, SPI_HandleTypeDef *spi_handler,
                 GPIO_TypeDef *conva_gpio_port, uint16_t conva_gpio_pin,
                 GPIO_TypeDef *convb_gpio_port, uint16_t convb_gpio_pin,
                 GPIO_TypeDef *busy_gpio_port, uint16_t busy_gpio_pin,
                 GPIO_TypeDef *reset_gpio_port, uint16_t reset_gpio_pin,
                 AD7606_InputRange volt_range, float volt_ref) {
  device->device_spi = spi_handler;
...
void AD7606_StartConv(AD7606_Handler *device, AD7606_Convst conv) {
  ...
  switch (conv) {
  case Convst_A:
    HAL_TIM_PWM_Start(device->conva_tim, device->conva_tim_channel);
    break;
  case Convst_B:
    HAL_TIM_PWM_Start(device->convb_tim, device->convb_tim_channel);
    break;
  case Convst_All:
    HAL_TIM_PWM_Start(device->conva_tim, device->conva_tim_channel);
    HAL_TIM_PWM_Start(device->convb_tim, device->convb_tim_channel);
...
void AD7606_StopConv(AD7606_Handler *device, AD7606_Convst conv) {
  switch (conv) {
  case Convst_A:
    HAL_TIM_PWM_Stop(device->conva_tim, device->conva_tim_channel);
    break;
  case Convst_B:
    HAL_TIM_PWM_Stop(device->convb_tim, device->convb_tim_channel);
    break;
  case Convst_All:
    HAL_TIM_PWM_Stop(device->conva_tim, device->conva_tim_channel);
    HAL_TIM_PWM_Stop(device->convb_tim, device->convb_tim_channel);
...
void AD7606_StartReadBytes(AD7606_Handler *device, int16_t *pDst,
                           uint16_t length) {
//   while (ad7606_dma_status == AD7606_DMA_BUSY)
//     ;
  ...
  while (HAL_GPIO_ReadPin(device->busy_port, device->busy_pin) == GPIO_PIN_SET)
    ;
  ad7606_dma_status = AD7606_DMA_BUSY;
  HAL_SPI_Receive(device->device_spi, (uint8_t *)pDst, length, 50);
  return;
}

void AD7606_ConvertToVoltage(AD7606_Handler *device, int16_t *pSrc, float *pDst,
                             uint16_t length) {
  uint16_t i;
  switch (device->range) {
  case Range_10VPP:
